...
class Juego {
    constructor() {
        ...
        if (this.imagenes===0) {
            ...
            this.colorGano="rgba(192, 64, 64, 0.824)"; //color Rosa
            this.colorAlreves="rgba(192, 128, 128, 0.824)";
            this.colorHallar.style.background="rgba(255, 128, 128, 1.1)";
        }else if (this.imagenes===1) {
            this.valorImagen = ['pocionAmarilla', 'pocionAnaranjado', 'pocionVerde', 'pocionBlanca', 'pocionMorada'];
            this.colorGano="rgba(128, 64, 128, 0.824)"  //color Lila
            this.colorAlreves="rgba(160, 128, 160, 0.824)"
            this.colorHallar.style.background='rgba(192, 128, 192, 1.1)'; 
        } else if(this.imagenes===2) {
            this.valorImagen = ['pocionAmarilla', 'pocionBlanca', 'pocionVerde', 'pocionRoja', 'pocionAzul'];
            this.colorAlreves="rgba(192, 128, 0, 0.824)";
            this.colorGano="rgba(192, 64, 0, 0.824)"; //color anaranjado
            this.colorHallar.style.background="rgba(255, 128, 0, 1.1)";
        } else{
    ...
    procesarDrop(id) {
        ...
        if (this.imagenes===0) {
            if (id !== 'pocionRoja' && id !== 'pocionBlanca') {
            this.corazones.perderCorazon();
            setTimeout(() => this.colorMezcla.reiniciar(), 1000);
            }
        }else if (this.imagenes===1) {
            if (id !== 'pocionBlanca' && id !== 'pocionMorada') {
                this.corazones.perderCorazon();
                setTimeout(() => this.colorMezcla.reiniciar(), 1000);
            }
        }else if (this.imagenes===2) {
            if (id !== 'pocionRoja' && id !== 'pocionAmarilla') {
                this.corazones.perderCorazon();
                setTimeout(() => this.colorMezcla.reiniciar(), 1000);
            }
        }
